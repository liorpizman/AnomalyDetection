...
from utils.helper_methods import get_threshold, report_results, get_method_scores, get_subdirectories, \
    ...
    plot_prediction_performance, get_plots_key, get_auc_plot_key, calculate_auc, get_estimator, tuning_auc_and_delay
...
def run_model(training_data_path, test_data_path, results_path, similarity_score, save_model, new_model_running,
    ...
    for flight_route in FLIGHT_ROUTES:
        ...
        if new_model_running:
            svr_model, X_train_scaler, Y_train_scaler, X_train, Y_train = execute_train(flight_route,
    ...
    for attack in ATTACKS:
        ...
        for flight_csv in os.listdir(f"{attacks_path}"):
            ...
            if add_plots:
                ...
                key = get_plots_key(algorithm='SVR', similarity=similarity_score, flight_route=flight_route)
